...
app.get('/api/health', (req, res) => {
  res.json({ 
    status: 'ok', 
    message: 'Poopalotzi Marina Management System running',
...
app.get('*', (req, res) => {
  ...
  const publicIndex = path.join(clientPublicPath, 'index.html');
  
  res.sendFile(distIndex, (err) => {
    if (err) {
      res.sendFile(publicIndex, (err2) => {
        if (err2) {
          res.status(404).send('Marina Management System - Page not found');
        }
      });
    }
  });
