...
export class CloverService {
  ...
  /**
   * Standalone refresh token function by merchant ID
   */
  static async refreshToken(merchantId: string): Promise<{ access_token: string, refresh_token: string }> {
    const { storage } = await import('./index');
    
    // Load current config for this merchant
    const cloverConfig = await storage.getCloverConfig();
    if (!cloverConfig || cloverConfig.merchantId !== merchantId) {
      throw new Error(`No Clover configuration found for merchant ${merchantId}`);
    }
    ...
    // Atomically update the stored tokens
    const expiresAt = new Date(Date.now() + (tokenData.expires_in * 1000));
    await storage.updateCloverConfig(cloverConfig.id, {
      accessToken: tokenData.access_token,
      refreshToken: tokenData.refresh_token || cloverConfig.refreshToken,
      tokenExpiresAt: expiresAt
    });
    
    return {
      access_token: tokenData.access_token,
      refresh_token: tokenData.refresh_token || cloverConfig.refreshToken
    };
  }
